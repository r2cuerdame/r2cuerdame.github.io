...
_Last updated: 2026-05-02T18:05:01+09:00 KST_
...
- 매일 포스팅은 문제가 없다. 조건은 명확하다: **각 글의 타겟이 선명하고, Reader QA + 타겟 품질 게이트 + 정적/편집 가드 + live layout QA + hard/soft issue 0건을 모두 통과해야 한다.**
...
- Reader QA + 타겟 품질 게이트 + 정적/편집 가드 + live layout QA 통과 + hard/soft issue 0건이면 `검사 충분`으로 본다. clean 후보는 7일 숙성이나 추가 완벽주의를 이유로 묶어두지 않는다. daily baseline은 1개, adaptive 발행부는 clean 미발행 큐가 목표 7개를 초과할 때 하루 최대 4개·최소 2시간 간격으로 추가 공개한다.
...
10. GitHub Pages 공개 전후로 `scripts/audit_public_site_quality.py`를 통과해야 한다. 이 게이트는 대표 공개 글/랜딩/검색 페이지의 타이틀·메타·복귀 링크·시각 리듬·본문 폭 보호 CSS·placeholder/충돌 마커를 함께 본다.
11. 로컬 빌드 후 `/radar/`, 수정된 글/딜 상세, CSS·템플릿 영향 페이지를 데스크톱과 390px 모바일에서 직접 확인했고, 한글 세로깨짐/겹침/잘림/페이지 overflow/가짜 이미지 텍스트가 0건이다.
12. GitHub Pages 공개 후 cache-busted live URL에서 데스크톱과 390px 모바일을 다시 확인했고, 최종 보고에는 확인 페이지·폭·증거를 남긴다. live에서 깨지면 사용자 보고 전 먼저 수정·재배포한다.
13. daily baseline 모드에서는 하루 1개를 넘지 않는다. adaptive 발행부 모드에서는 clean 미발행 큐가 목표 7개를 초과하고 spacing guard가 열렸을 때 하루 최대 4개까지만 공개한다.
...
- 공개 사이트 레이아웃/최종 독자 품질 오류: `scripts/audit_public_site_quality.py` 실패 사유를 먼저 고치고, `layout_review_required`로 보강한 뒤 로컬 build·브랜드 안전·검색 smoke·desktop/390px·라이브 품질 audit까지 다시 통과하기 전에는 공개 완료로 보고하지 않는다.
