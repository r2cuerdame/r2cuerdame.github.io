...
11. GitHub Pages 공개 전후로 `scripts/audit_public_site_quality.py`를 통과해야 한다. 이 게이트는 대표 공개 글/랜딩/검색 페이지의 타이틀·메타·복귀 링크·시각 리듬·본문 폭 보호 CSS·placeholder/충돌 마커를 함께 본다.
...
- 공개 사이트 레이아웃/최종 독자 품질 오류: `scripts/audit_public_site_quality.py` 실패 사유를 먼저 고치고, 로컬 build·브랜드 안전·검색 smoke·라이브 품질 audit까지 다시 통과하기 전에는 공개 완료로 보고하지 않는다.
