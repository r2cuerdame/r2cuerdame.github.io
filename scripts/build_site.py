...
def load_seoul_map_outline() -> dict[str, Any]:
    if not SEOUL_MAP_OUTLINE_PATH.exists():
        return {}
    try:
        data = json.loads(SEOUL_MAP_OUTLINE_PATH.read_text(encoding="utf-8-sig"))
    except Exception:
        return {}
    if not isinstance(data, dict) or not data.get("districts"):
        return {}
    return data
...
def seoul_station_map_position(station: dict[str, Any]) -> tuple[float, float]:
    ...
    bounds = outline.get("bounds") if isinstance(outline, dict) else None
    padding = outline.get("padding") if isinstance(outline, dict) else None
    try:
        lon = float(station.get("lng"))
        lat = float(station.get("lat"))
        width = float(outline.get("width") or 1000)
        height = float(outline.get("height") or 680)
        pad_x = float((padding or {}).get("x", 46))
        pad_y = float((padding or {}).get("y", 44))
        ...
        return max(2.5, min(97.5, x / width * 100)), max(2.5, min(97.5, y / height * 100))
    except Exception:
...
def seoul_map_outline_svg() -> str:
    outline = SEOUL_MAP_OUTLINE
    if not outline:
        return '<div class="seoul-real-map seoul-real-map-empty" aria-hidden="true"></div>'
    view_box = esc(str(outline.get("view_box") or "0 0 1000 680"))
    station_districts = {str(station.get("district", "")) for station in SEOUL_COMMERCIAL_AREAS["stations"]}
    district_paths = []
    district_labels = []
    for district in outline.get("districts", []):
        ...
        if name in station_districts:
            ...
            district_labels.append(
                f'<text class="seoul-district-label" x="{float(district.get("label_x", 0)):.1f}" y="{float(district.get("label_y", 0)):.1f}">{esc(label)}</text>'
            )
    river = outline.get("river") if isinstance(outline.get("river"), dict) else {}
    river_path = esc(str(river.get("path") or ""))
    return f'''
      <svg class="seoul-real-map" viewBox="{view_box}" preserveAspectRatio="none" role="img" aria-label="서울 25개 자치구 실제 행정경계와 OSM 한강 outline">
        <defs>
...
<section id="commercial-check-tool" class="seoul-density-panel" data-seoul-density-tool-root data-commercial-tool-root data-density-src="/data/seoul-commercial-areas.json?v={data_version}" aria-label="서울 역세권 업종 밀도 지도">
  ...
  <div class="seoul-map-card" data-seoul-map>
    ...
    <aside class="map-reading-guide" data-map-reading-guide aria-label="서울 상권 지도 읽는 순서">
      <strong>지도 읽는 순서</strong>
      <ol>
        <li><span>1</span>업종 탭으로 원 크기를 바꿉니다.</li>
        <li><span>2</span>후보역을 눌러 같은 반경을 비교합니다.</li>
        <li><span>3</span>오른쪽 첫 질문에 답 없으면 보류합니다.</li>
      </ol>
    </aside>
  </div>
...
.seoul-density-panel {
  ...
  display: grid; grid-template-columns: minmax(620px, 1.22fr) minmax(380px, .78fr); gap: clamp(18px, 2.4vw, 30px);
  align-items: start; margin: 12px 0 46px; padding: clamp(22px, 3.2vw, 38px);
  border: 1px solid rgba(255,255,255,.10); border-radius: 38px;
  background:
    radial-gradient(circle at 13% 6%, rgba(255,90,31,.24), transparent 30%),
    radial-gradient(circle at 82% 8%, rgba(96,165,250,.24), transparent 34%),
    linear-gradient(145deg, #080b14 0%, #0f172a 54%, #101828 100%);
  color: #f8fafc;
  box-shadow: 0 36px 90px rgba(15,23,42,.26), inset 0 1px 0 rgba(255,255,255,.08);
  overflow: hidden; isolation: isolate;
}
.seoul-density-panel::before { content: ""; position: absolute; inset: 88px 24px auto auto; z-index: -1; width: min(48vw, 560px); height: 560px; border-radius: 50%; background: radial-gradient(circle, rgba(125,211,252,.12), transparent 68%); filter: blur(2px); }
.seoul-density-panel::after { content: ""; position: absolute; left: 30px; right: 30px; top: 116px; z-index: -1; height: 1px; background: linear-gradient(90deg, transparent, rgba(255,255,255,.18), transparent); }
.seoul-tool-copy, .seoul-map-card, .station-inspector, .density-result-card, .density-compare-card { min-width: 0; }
.seoul-tool-copy { grid-column: 1 / -1; display: grid; grid-template-columns: minmax(0, 1fr) minmax(260px, auto); gap: 20px; align-items: end; max-width: none; padding: 2px 4px 0; }
.seoul-tool-copy .eyebrow { color: #fb923c; }
.seoul-tool-copy h2 { max-width: 860px; margin: 6px 0 12px; color: #fff; font-size: clamp(36px, 4.9vw, 66px); line-height: .98; letter-spacing: -0.064em; text-wrap: balance; }
.seoul-tool-copy p { max-width: 760px; margin: 0; color: #cbd5e1; font-size: clamp(15px, 1.35vw, 18px); line-height: 1.62; font-weight: 780; }
.tool-badges { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 8px; margin-top: 0; }
.tool-badges span { display: inline-flex; align-items: center; min-height: 34px; padding: 0 12px; border-radius: 999px; background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.12); color: #e2e8f0; font-size: 12px; font-weight: 950; box-shadow: inset 0 1px 0 rgba(255,255,255,.06); backdrop-filter: blur(10px); }
.seoul-map-card { position: relative; align-self: start; display: flex; flex-direction: column; min-height: 0; padding: clamp(18px, 2.2vw, 26px); border-radius: 32px; background: linear-gradient(180deg, rgba(15,23,42,.96) 0%, rgba(8,13,26,.98) 100%); color: #fff; box-shadow: inset 0 0 0 1px rgba(255,255,255,.10), 0 24px 54px rgba(0,0,0,.28); overflow: hidden; }
.seoul-map-card::before { content: ""; position: absolute; inset: -30% -20% auto 28%; height: 330px; background: radial-gradient(circle, rgba(96,165,250,.22), transparent 64%); pointer-events: none; }
...
.map-card-head strong { margin-top: 3px; color: #fff; font-size: clamp(24px, 2.25vw, 34px); line-height: 1.05; letter-spacing: -.05em; }
.map-card-head small { max-width: 300px; color: #cbd5e1; font-size: 12px; line-height: 1.45; font-weight: 850; text-align: right; }
...
.map-reading-guide { position: relative; z-index: 2; margin: 12px 2px 0; padding: 14px; border-radius: 20px; background: rgba(255,255,255,.07); border: 1px solid rgba(255,255,255,.13); box-shadow: inset 0 1px 0 rgba(255,255,255,.08); }
.map-reading-guide strong { display: block; color: #fff; font-size: 13px; line-height: 1.25; font-weight: 950; letter-spacing: -.015em; }
.map-reading-guide ol { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 7px; margin: 10px 0 0; padding: 0; list-style: none; }
.map-reading-guide li { min-width: 0; display: flex; gap: 7px; align-items: flex-start; padding: 9px; border-radius: 14px; background: rgba(2,6,23,.34); color: #dbeafe; font-size: 11.5px; line-height: 1.4; font-weight: 850; word-break: keep-all; }
.map-reading-guide li span { flex: 0 0 auto; display: grid; place-items: center; width: 19px; height: 19px; border-radius: 999px; background: #ff5a1f; color: #fff; font-size: 10px; line-height: 1; font-weight: 950; box-shadow: 0 0 0 4px rgba(255,90,31,.12); }
...
.seoul-selector-form select { width: 100%; min-height: 46px; border: 1px solid rgba(255,255,255,.12); border-radius: 15px; padding: 0 12px; background: rgba(255,255,255,.06); color: #f8fafc; font: inherit; font-weight: 850; }
.seoul-selector-form select option { color: #111827; background: #fff; }
.density-result-card { padding: 20px; border-radius: 28px; background: rgba(248,250,252,.96); border: 1px solid rgba(255,255,255,.72); box-shadow: 0 18px 44px rgba(0,0,0,.16); }
.density-title-row { display: flex; justify-content: space-between; gap: 14px; align-items: flex-start; }
.density-title-row h3 { margin: 7px 0 6px; color: #0f172a; font-size: clamp(23px, 2.4vw, 31px); line-height: 1.1; letter-spacing: -.045em; }
...
.density-result-card p { margin: 0 0 13px; color: #475569; font-weight: 760; }
...
.density-score-grid strong { display: block; margin: 2px 0; color: #0f172a; font-size: clamp(25px, 4vw, 38px); line-height: 1; letter-spacing: -.055em; }
.density-bars { display: grid; gap: 8px; margin: 12px 0 15px; }
.density-bar { display: grid; grid-template-columns: 78px 1fr 42px; gap: 8px; align-items: center; font-size: 12px; font-weight: 950; color: #334155; }
.density-bar span:nth-child(2) { height: 9px; border-radius: 999px; background: #e2e8f0; overflow: hidden; }
...
.density-compare-card > strong { display: block; margin: 9px 0 10px; color: #fff; font-size: clamp(20px, 2.1vw, 25px); line-height: 1.18; letter-spacing: -.04em; }
...
@media (min-width: 1121px) {
  ...
  .seoul-selector-form { grid-column: 2; grid-row: 2; align-self: start; }
  .density-compare-card { grid-column: 2; grid-row: 2; align-self: end; }
...
@media (max-width: 860px) {
  ...
  .map-reading-guide { padding: 12px; border-radius: 18px; }
  .map-reading-guide ol { grid-template-columns: 1fr; gap: 6px; }
  .map-reading-guide li { min-height: 40px; font-size: 11.5px; }
