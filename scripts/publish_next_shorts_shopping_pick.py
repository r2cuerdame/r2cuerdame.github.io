...
def short_body(candidate: dict[str, Any]) -> str:
    ...
    return (
        ...
        '<h2>바로 볼 3가지</h2>'
        f'<ol>{check_items}</ol>'
        '<p>첫째, 실제 사용 빈도를 정하세요. 매일 쓰는 소모품인지, 가끔 문제 생길 때만 찾는 보조템인지에 따라 필요한 수량과 가격 기준이 달라집니다. '
        '둘째, 설치나 보관 위치를 정하세요. 놓을 자리와 교체 동선이 맞지 않으면 가격이 좋아도 계속 쓰기 어렵습니다. '
        '셋째, 후기를 볼 때는 장점보다 반복되는 불만을 먼저 보세요. 냄새, 내구성, 크기, 배송 상태처럼 자주 언급되는 조건이 구매 실패를 줄입니다.</p>'
        '<h2>후보 3개 빠른 비교</h2>'
        ...
        '<p>상품 페이지에서는 대표 이미지보다 규격, 구성 수량, 재질, 배송비, 반품비를 먼저 보세요. '
        '같은 이름의 상품도 옵션과 묶음 수량이 다르면 실제 단가와 사용감이 달라집니다.</p>'
        '<p>가족이 함께 쓰면 교체 주기와 보관 위치가 중요하고, 혼자 쓰면 과하게 많은 구성보다 보관 부담이 작은 쪽이 낫습니다. '
        '특히 생활용품은 한 번에 많이 사기보다 실제 공간과 사용 속도에 맞는 수량을 고르는 편이 안전합니다.</p>'
        '<p>짧은 체크 글이라도 사용 위치와 관리 습관까지 맞아야 계속 쓰게 됩니다. '
        '설치할 자리, 버릴 때 동선, 재구매 주기가 아직 정해지지 않았다면 구매를 하루 미루고 옵션을 다시 보는 편이 낫습니다.</p>'
        '<h2>한 줄 정리</h2>'
        '<p>이 숏츠 쇼핑픽은 가격보다 사용 빈도, 설치와 보관 조건, 교체 주기가 먼저입니다. '
