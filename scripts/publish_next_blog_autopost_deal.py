...
def repo_candidate_body(candidate: dict[str, Any]) -> str:
    ...
    product_type = str(candidate.get('product_type') or candidate.get('topic') or title or '생활템').strip()
    intro = str(candidate.get('intro') or '').strip()
    use_case = str(candidate.get('use_case') or '').strip()
    criteria = candidate.get('decision_criteria') if isinstance(candidate.get('decision_criteria'), list) else []
    criteria = [str(x).strip() for x in criteria if str(x).strip()][:5]
    criteria_text = ', '.join(criteria) if criteria else '사용 빈도, 보관 위치, 관리 부담'
    rows = []
    sections = []
    for index, product in enumerate(products, start=1):
        ...
        sections.append(
            ...
            f'{html.escape(product_type)}은 실제 사용 위치, 교체·세척 주기, 보관 부담처럼 매일 반복되는 조건이 맞지 않으면 가격이 좋아도 보류하는 편이 안전합니다.</p>'
            f'<p>이 후보를 볼 때는 대표 이미지보다 {html.escape(criteria_text)}를 먼저 대입해 보세요. '
            '한 번 쓰고 끝나는 물건이 아니라 반복해서 꺼내 쓰거나 채워 두는 생활용품일수록 설치와 정리 과정이 만족도를 크게 가릅니다. '
            '상품 상세의 옵션명, 구성 수량, 실제 크기, 소재, 배송비를 함께 봐야 결제 직전 실패를 줄일 수 있습니다.</p>'
            f'<p>반대로 {html.escape(product_type)} 하나로 모든 문제가 해결될 것처럼 기대한다면 보류가 낫습니다. '
            '소모품은 교체 주기와 보관 위치가 맞아야 하고, 도구형 상품은 청소와 정리 동선이 맞아야 계속 쓰게 됩니다. '
            '그래서 상품 상세에서는 할인율보다 최근 후기의 불만 패턴, 구성품 누락 여부, 반품비 조건을 먼저 확인하세요.</p>'
            f'<ul>{check_items}</ul>'
    ...
    return (
        ...
        '가격이 낮아도 매번 꺼내기 불편하면 결국 방치되고, 반대로 과한 구성은 보관 장소가 없으면 만족도가 떨어집니다.</p>'
        f'<p>두 번째 기준은 {html.escape(criteria[1] if len(criteria) > 1 else "설치·보관 위치")}입니다. 실제로 놓을 자리가 있는지, 꺼내고 넣는 동선이 짧은지, 물기·먼지·열처럼 주변 환경의 영향을 받는지 확인해야 합니다. '
        '사용 장소가 정해지지 않았다면 상품 상세보다 집 안 동선을 먼저 보는 편이 좋습니다.</p>'
        f'<p>세 번째 기준은 {html.escape(criteria[2] if len(criteria) > 2 else "교체와 관리 부담")}입니다. 소모품은 얼마나 자주 바꿔야 하는지, 도구형 상품은 세척과 정리가 쉬운지에 따라 실제 비용이 달라집니다. '
        '구매 전에 구성 수량, 소재, 세척 가능 여부, 반품비 조건을 확인하면 실패 확률을 줄일 수 있습니다.</p>'
        f'<p>마지막 기준은 기대치입니다. {html.escape(product_type)}은 생활의 불편을 줄이는 보조 도구이지 모든 문제를 한 번에 없애는 만능템이 아닙니다. '
        '이미 생긴 냄새, 찌든 때, 설치 공간 부족처럼 다른 관리가 필요한 문제는 별도로 봐야 합니다.</p>'
        '<p>비교할 때는 후보를 세 등급으로 나누면 편합니다. 첫째는 부담 없이 써볼 입문형, 둘째는 반복 사용을 전제로 한 실속형, 셋째는 구성이나 내구성을 더 보는 확장형입니다. '
        '이 구분 없이 가격만 낮은 순서로 보면 배송비를 더한 실제 결제 금액, 보관 부피, 교체 주기가 뒤늦게 보입니다. '
        '특히 판매 페이지의 대표 사진은 실제 크기와 구성품을 축약해 보여줄 수 있으니 상세 치수와 옵션명을 꼭 확인하세요.</p>'
        '<p>후보를 고른 뒤에는 바로 결제하지 말고 실제 사용 장면에 대입해 보세요. 어디에 두고 쓸지, 누가 주로 쓸지, 다 쓰고 난 뒤 어디에 보관할지에 따라 필요한 조건이 달라집니다. '
        '가족이 함께 쓰면 구성 수량과 교체 주기가 중요하고, 혼자 쓰면 꺼내는 속도와 보관 부담이 더 중요합니다.</p>'
        '<p>생활용품은 작은 습관 차이가 오래 씁니다. 쓰기 전후 손이 많이 가거나, 물기와 쓰레기를 처리할 동선이 불편하거나, 보관 위치가 애매하면 좋은 상품도 금방 방치됩니다. 구매 전 그 과정을 한 번 상상해 보는 것이 가장 실용적인 검수입니다.</p>'
        '<h2>제품별 핵심 포인트</h2>'
        f'{"".join(sections)}'
        f'{table}{faq}'
        '<h2>정리</h2>'
        f'<p>{html.escape(product_type)}을 고를 때는 가격보다 실제 사용 장면과 관리 부담을 먼저 맞추는 편이 안전합니다. '
        '상품 페이지에서는 옵션명, 구성 수량, 배송비, 반품비, 최근 후기를 마지막으로 확인한 뒤 결정하세요.</p>'
        f'{disclosure}'
        '</div>'
    )
